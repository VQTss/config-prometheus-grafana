...
set_tracer_provider(
    TracerProvider(resource=Resource.create({SERVICE_NAME: "yolov5-service"}))
)

trace = get_tracer_provider().get_tracer('detections',"0.1.2")

jeager_expoter = JaegerExporter(
    agent_host_name="localhost",
    agent_port=6831
)

span_processor = BatchSpanProcessor(jeager_expoter)

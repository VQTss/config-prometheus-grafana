...
set_tracer_provider(
    TracerProvider(
        resource=Resource.create(
            {
                SERVICE_NAME: "yolov5-manual-service"
            }
        )
    )
)

tracer = get_tracer_provider().get_tracer('detections',"0.1.2")
...
get_tracer_provider().add_span_processor(span_processer)
...
with tracer.start_as_current_span(
    ...
    model = get_model_YOLOv5() # Time model load checkpoint
...
async def od_macular_detection(image_url: str = Query(..., description="URL of the image to process.")):
    ...
    try:
        with tracer.start_as_current_span("processors") as processors: # Check all flow in event
            with tracer.start_as_current_span(
            ...
            with tracer.start_as_current_span(
                ...
                # model predict from input images
